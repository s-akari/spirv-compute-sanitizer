...
int run_kernel(OpenCLContext *ctx, OpenCLBuffers *buffers) {
  ...
  err = clSetKernelArg(ctx->kernel, 1, sizeof(unsigned long), &size);
  CHECK_CL_ERROR(err, "Error in setting kernel argument size");

  err = clSetKernelArg(ctx->kernel, 2, sizeof(cl_mem), &buffers->d_b);
  CHECK_CL_ERROR(err, "Error in setting kernel argument d_b");

  err = clSetKernelArg(ctx->kernel, 3, sizeof(unsigned long), &size);
  CHECK_CL_ERROR(err, "Error in setting kernel argument size");

  err = clSetKernelArg(ctx->kernel, 4, sizeof(cl_mem), &buffers->d_c);
  CHECK_CL_ERROR(err, "Error in setting kernel argument d_c");

  err = clSetKernelArg(ctx->kernel, 5, sizeof(unsigned long), &size);
  CHECK_CL_ERROR(err, "Error in setting kernel argument size");
